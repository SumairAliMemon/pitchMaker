    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Manage your personal information and background</p>
            </div>
          </div>
          <button
        ...
        <div className="space-y-6">
          ...
          {/* Background Details */}
          <div>
            <label htmlFor="background_details" className="block text-sm font-semibold text-gray-700 mb-3">
              Background Details
            </label>
            {editing ? (
              <textarea
          ...
          {editing && (
            <div className="flex justify-end space-x-3 pt-4">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex items-center space-x-2 px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl transition-colors duration-200 disabled:opacity-50"
              >
                {saving ? (
                  <>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-3">
              <CheckCircle className={`w-5 h-5 ${userProfile.full_name ? 'text-green-500' : 'text-gray-300'}`} />
              <span className={`text-sm ${userProfile.full_name ? 'text-green-700' : 'text-gray-500'}`}>
                Full Name
              </span>
            </div>
            <div className="flex items-center space-x-3">
              <CheckCircle className={`w-5 h-5 ${userProfile.background_details ? 'text-green-500' : 'text-gray-300'}`} />
              <span className={`text-sm ${userProfile.background_details ? 'text-green-700' : 'text-gray-500'}`}>
                Background Details
              </span>
            </div>
            <div className="flex items-center space-x-3">
              <CheckCircle className={`w-5 h-5 ${userProfile.skills ? 'text-green-500' : 'text-gray-300'}`} />
              <span className={`text-sm ${userProfile.skills ? 'text-green-700' : 'text-gray-500'}`}>
                Skills
              </span>
            </div>
            <div className="flex items-center space-x-3">
              <CheckCircle className={`w-5 h-5 ${userProfile.experience ? 'text-green-500' : 'text-gray-300'}`} />
              <span className={`text-sm ${userProfile.experience ? 'text-green-700' : 'text-gray-500'}`}>
                Experience
              </span>
            </div>
            <div className="flex items-center space-x-3">
              <CheckCircle className={`w-5 h-5 ${userProfile.education ? 'text-green-500' : 'text-gray-300'}`} />
              <span className={`text-sm ${userProfile.education ? 'text-green-700' : 'text-gray-500'}`}>
                Education
              </span>
            </div>
          </div>
